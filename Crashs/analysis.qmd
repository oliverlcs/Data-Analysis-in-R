...
**Nächtliche Unfallschwere:** Die Hypothese, dass nächtliche Unfälle trotz geringerer Häufigkeit schwerere Verletzungen verursachen, wird durch die Daten gestützt. Der Anteil schwerer Verletzungen (FATAL + SUSPECTED SERIOUS INJURY) liegt nachts (20–6 Uhr) bei 1,38% gegenüber 0,78% am Morgen, 0,79% am Mittag und 0,72% am Abend – ein nahezu doppelt so hohes relatives Risiko. Der Chi-Quadrat-Test (p < 2.2e-16) bestätigt diesen Zusammenhang statistisch.
...
Die statistische Auswertung liefert ein auf den ersten Blick kontraintuitives Ergebnis: Der Anteil schwerer Verletzungen fällt bei „Gutem Wetter“ mit 0,860 % höher aus als bei „Schlechtem Wetter“ (0,789 %). Obwohl die absoluten Fallzahlen bei gutem Wetter deutlich höher liegen, ist das relative Risiko für schwerste Unfallfolgen bei widrigen Bedingungen geringer. Dies stützt die Annahme, dass Verkehrsteilnehmer bei Regen, Schnee oder Nebel durch angepasste Geschwindigkeit und erhöhte Vorsicht die Schwere der Aufprallenergie reduzieren, während bei optimalen Sicht- und Straßenverhältnissen tendenziell höhere Geschwindigkeiten gefahren werden, die im Unfallfall zu gravierenderen Verletzungen führen.
...
**Verletzungsschwere:** Der Vergleich zeigt, dass Unfälle bei gutem Wetter tatsächlich einen höheren Anteil schwerer Verletzungen aufweisen (0,86%) als bei schlechtem Wetter (0,79%). Besonders auffällig: Bei Regen (0,67%) und Schnee (0,60%) ist das Risiko schwerer Verletzungen sogar niedriger als bei klarem Himmel. Die höchste Quote schwerer Verletzungen findet sich bei Nebel/schlechter Sicht (1,06%), wo die Gefahr offenbar unterschätzt wird.
...
**Interpretation:** Diese Befunde stützen die Hypothese der Risikokompensation: Fahrer passen ihr Verhalten bei offensichtlich widrigen Bedingungen an (geringere Geschwindigkeit, erhöhte Aufmerksamkeit), während bei vermeintlich guten Verhältnissen tendenziell schneller und unvorsichtiger gefahren wird. Der Chi-Quadrat-Test (p = 3,0e-12) bestätigt den Zusammenhang statistisch.
...
Die Grafik verdeutlicht eine fundamentale Einschränkung der vorliegenden Datenbasis: Mit 204.688 erfassten Fällen stellt „VISUAL DISTRACTION“ die einzig verfügbare Kategorie dar. Es findet keine weitere Differenzierung statt (z. B. in auditiv, kognitiv oder spezifische Auslöser wie Handynutzung). Dies erklärt auch rückblickend das Ergebnis der vorangegangenen Hypothesenprüfung: Da der Datensatz sämtliche Formen der Ablenkung unter diesem generalisierten Oberbegriff subsumiert, ist eine isolierte statistische Auswertung spezifischer Störquellen (wie Mobiltelefonen) mit diesen Daten nicht möglich.
...
Die resultierende Heatmap für die Kategorie „VISUAL DISTRACTION“ offenbart ein eindeutiges Risikomuster. Mit einem Anteil von 33 % stellt der Auffahrunfall („REAR-END“) die mit Abstand häufigste Unfallfolge bei visueller Ablenkung dar. Dieser Befund ist inhaltlich plausibel, da der fehlende Blick auf das Verkehrsgeschehen (z. B. durch Smartphone-Nutzung) die Reaktionszeit auf bremsende vorausfahrende Fahrzeuge drastisch verkürzt. An zweiter Stelle folgen Winkelkollisionen („ANGLE“) mit 20 %, was darauf hindeutet, dass abgelenkte Fahrer zudem häufig Vorfahrtsregelungen oder Ampelsignale im Kreuzungsbereich übersehen. Andere Kollisionstypen wie „SIDESWIPE“ (12 %) oder Alleinunfälle („SINGLE VEHICLE“, 10 %) spielen eine vergleichsweise geringere Rolle.

#### Ablenkung und Schuld

```{r}
# Vergleich: Abgelenkte vs. nicht abgelenkte Fahrer
distraction_fault_comparison <- data %>%
  filter(!is.na(Driver.At.Fault)) %>%
  mutate(
    Ablenkungsstatus = if_else(!is.na(Driver.Distracted.By), "Abgelenkt", "Nicht abgelenkt")
  ) %>%
  group_by(Ablenkungsstatus) %>%
  summarise(
    n = n(),
    anteil_schuld = mean(Driver.At.Fault) * 100
  )
...
# Visualisierung
ggplot(distraction_fault_comparison, aes(x = Ablenkungsstatus, y = anteil_schuld, fill = Ablenkungsstatus)) +
  geom_col(width = 0.7) +
  geom_text(aes(label = paste0(round(anteil_schuld, 1), "%")), vjust = -0.5, size = 4) +
  labs(
    x = "Status",
    y = "Anteil schuldiger Fahrer (%)",
    title = "Einfluss von Ablenkung auf die Unfallschuld",
    fill = "Status"
  ) +
  theme_minimal() +
  theme(legend.position = "none") +
...
Zur Analyse des Einflusses von Ablenkung auf die Unfallverantwortlichkeit wurde der Anteil schuldiger Fahrer innerhalb der Kategorie „Abgelenkt (Visual)“ ermittelt.

Das Ergebnis zeigt, dass 51,6 % der Fahrer, bei denen eine visuelle Ablenkung dokumentiert wurde, als unfallverursachend eingestuft wurden. Da in der Darstellung keine Vergleichsgruppe (z. B. „Keine Ablenkung“) abgebildet ist, lässt sich aus diesem isolierten Wert statistisch nicht ableiten, ob Ablenkung das Unfallrisiko signifikant erhöht. Der Wert liegt nur knapp über dem Zufallsmittel, was ohne einen Referenzwert für aufmerksame Fahrer keine belastbare Aussage über eine relative Risikoerhöhung zulässt. Das Fehlen des zweiten Balkens deutet darauf hin, dass die Datenbasis für diese Grafik vorab auf Einträge mit vorhandenem Ablenkungsvermerk gefiltert wurde.

#### Zwischenfazit Fragestellung D

Die Analyse der Ablenkung am Steuer offenbart eine wesentliche Limitation der Datenbasis:

**Datenlimitation:** Die Spalte „Driver.Distracted.By" enthält nach der Datenbereinigung ausschließlich den Wert „VISUAL DISTRACTION" für alle 204.688 Fälle. Es findet keine weitere Differenzierung in spezifische Ablenkungsarten (z.B. Handynutzung, manuelle Tätigkeiten, Gespräche) statt. Dies bedeutet, dass die ursprüngliche Hypothese – Handynutzung sei die häufigste dokumentierte Ablenkungsform – mit diesem Datensatz nicht überprüft werden kann.

**Kollisionstypen bei visueller Ablenkung:** Trotz der eingeschränkten Datenbasis liefert die Analyse wertvolle Erkenntnisse. Bei dokumentierter visueller Ablenkung dominieren erwartungsgemäß Auffahrunfälle (REAR-END) mit 33% aller Fälle. Dies ist inhaltlich plausibel: Wer den Blick von der Straße abwendet, bemerkt bremsende Vordermänner zu spät. Winkelkollisionen (ANGLE) folgen mit 20%, was auf übersehene Vorfahrtsregelungen hindeutet.

**Ablenkung und Schuld:** Der Schuldanteil bei abgelenkten Fahrern (51,6%) liegt nur geringfügig über dem statistischen Erwartungswert. Ohne Vergleichsgruppe (Fahrer ohne dokumentierte Ablenkung) lässt sich hieraus jedoch keine belastbare Aussage über eine relative Risikoerhöhung ableiten.

**Fazit:** Die Hypothese zur Handynutzung kann nicht getestet werden. Die verfügbaren Daten zeigen jedoch, dass visuelle Ablenkung überproportional zu Auffahrunfällen führt – ein Teilaspekt der ursprünglichen Vermutung wird somit bestätigt.
...
Hier ist ein Entwurf für die Bildbeschreibung, der sich streng an dem von Ihnen vorgegebenen akademischen und erklärenden Stil orientiert:

Für die Analyse der Beteiligung nicht-motorisierter Verkehrsteilnehmer wurde die Spalte „Related.Non.Motorist“ herangezogen. Zunächst wurden sämtliche Datensätze, die in dieser Variable fehlende Werte (NA) aufwiesen, mittels einer Filterfunktion entfernt, um die Auswertung auf valide Einträge zu beschränken.

Die verbleibenden Daten wurden anschließend nach Kategorien gruppiert und hinsichtlich ihrer Häufigkeit ausgezählt (`count`), wobei eine absteigende Sortierung vorgenommen wurde. Dies dient dazu, die relevantesten Gruppen unmittelbar hervorzuheben. Für die Visualisierung wurde ein Balkendiagramm gewählt, dessen Achsen mittels `coord_flip()` vertauscht wurden. Diese Darstellung gewährleistet, dass die teils langen Kategorienbezeichnungen (wie „OCCUPANT OF MOTOR VEHICLE NOT IN TRANSPORT“) auf der y-Achse gut lesbar bleiben. Zudem wurden die absoluten Fallzahlen direkt an den Balken beschriftet, um einen präzisen Vergleich der Häufigkeiten zu ermöglichen.
...
Für den Vergleich der Lichtverhältnisse wurde zunächst die Spalte „Light“ bereinigt, indem Datensätze mit fehlenden Werten (NA) herausgefiltert wurden. Um eine Unterscheidung zwischen verschiedenen Unfallarten zu ermöglichen, wurde eine neue dichotome Variable generiert. Diese basiert auf der Spalte „Related.Non.Motorist“: Ist ein Eintrag vorhanden, wird der Unfall der Kategorie „Mit Fußgänger/Radfahrer“ zugeordnet; ist der Wert hingegen NA, fällt er in die Kategorie „Nur Fahrzeuge“.

Ein wesentlicher Schritt in der Datenaufbereitung war die Berechnung der relativen Anteile innerhalb dieser beiden Gruppen. Da sich die absoluten Fallzahlen stark unterscheiden, wurden die Häufigkeiten prozentual normiert, um die Verteilung der Lichtverhältnisse vergleichbar zu machen. Die Visualisierung erfolgte anschließend als gruppiertes Balkendiagramm (`position = "dodge"`), wodurch die Unterschiede in den Lichtverhältnissen zwischen den beiden Unfallkategorien direkt gegenübergestellt werden können. Zur besseren Lesbarkeit wurden die Kategorien auf der x-Achse um 45 Grad rotiert.
...
Für die spezifische Untersuchung des Anteils von Unfällen bei Dunkelheit wurde erneut auf die Spalte „Light“ zurückgegriffen. Zunächst wurden Datensätze mit fehlenden Werten (NA) ausgeschlossen, um eine bereinigte Basis für die Berechnung zu erhalten.

Ein vergleichbares Vorgehen wie in den vorangegangenen Schritten wurde bei der Klassifizierung der Unfallbeteiligten angewendet: Basierend auf der Spalte „Related.Non.Motorist“ wurde eine binäre Variable erstellt, die unterscheidet, ob nicht-motorisierte Verkehrsteilnehmer involviert waren oder ob es sich um reine Fahrzeugunfälle handelt.

Zusätzlich wurde die Variable `is_dark` definiert. Hierbei wurden mittels Textmustererkennung (`str_detect`) alle Einträge in der Spalte „Light“, die das Wort „DARK“ enthalten, identifiziert und zusammengefasst. Diese Methode erlaubt es, sämtliche Abstufungen von Dunkelheit (z. B. mit oder ohne Straßenbeleuchtung) in einer einzigen logischen Kategorie zu bündeln. Anschließend wurden die Daten gruppiert, um den relativen Anteil der Unfälle bei Dunkelheit (`anteil_dark`) für beide Gruppen zu berechnen und quantitativ vergleichbar zu machen.
...
Für die vergleichende Analyse der Verletzungsschwere diente die Spalte „Injury.Severity“ als Basis. In einem ersten Schritt wurden Datensätze mit fehlenden Werten (NA) aus dem Datensatz entfernt, um die Auswertung auf vollständig dokumentierte Fälle zu beschränken.

Ein vergleichbares Vorgehen wie in den vorangegangenen Analysen wurde bei der Gruppierung der Unfälle angewendet. Basierend auf der Variable „Related.Non.Motorist“ wurde unterschieden, ob nicht-motorisierte Verkehrsteilnehmer involviert waren oder ob es sich ausschließlich um Fahrzeugunfälle handelte.

Der entscheidende Schritt in der Datenaufbereitung lag in der Berechnung der relativen Anteile (`anteil`). Da die absolute Anzahl der reinen Fahrzeugunfälle deutlich höher ist als die der Unfälle mit Personenbeteiligung, war eine Normierung auf Prozentwerte notwendig, um die Verteilungen der Verletzungsschwere direkt gegenüberstellen zu können. Die grafische Darstellung als gruppiertes Balkendiagramm (`position = "dodge"`) verdeutlicht diese Unterschiede, wobei die Achsenbeschriftungen zur besseren Lesbarkeit der Kategorien erneut um 45 Grad rotiert wurden.
...
Für die spezifische Untersuchung des Risikos besonders schwerer Unfallfolgen wurde erneut auf die Spalte „Injury.Severity“ zurückgegriffen. Zunächst wurden Datensätze mit fehlenden Werten (NA) eliminiert, um die Analyse auf valide Einträge zu beschränken.

Ein vergleichbares Vorgehen wie in den vorangegangenen Schritten wurde bei der Klassifizierung der Unfallbeteiligten angewendet: Basierend auf der Spalte „Related.Non.Motorist“ wurde differenziert, ob nicht-motorisierte Verkehrsteilnehmer involviert waren oder ob es sich um reine Fahrzeugunfälle handelt.

Zusätzlich wurde die logische Variable `is_severe` definiert. Hierbei wurden die Ausprägungen „FATAL“ (tödlich) und „SUSPECTED SERIOUS INJURY“ (vermutlich schwere Verletzung) zusammengefasst. Diese Aggregation erlaubt es, den Fokus gezielt auf die schwerwiegendsten Unfallausgänge zu legen. Anschließend wurden die Daten gruppiert, um die absoluten Fallzahlen sowie den prozentualen Anteil dieser schweren Verletzungen (`anteil_severe`) innerhalb der beiden Vergleichsgruppen zu berechnen.
...
Für die Analyse der zeitlichen Verteilung von Unfällen mit nicht-motorisierter Beteiligung wurden die Spalten „Related.Non.Motorist“ und „Crash.Date.Time“ herangezogen. Zunächst wurden Datensätze, die in einer dieser Variablen fehlende Werte (NA) aufwiesen, mittels einer Filterfunktion entfernt, um die Auswertung auf valide Fälle zu beschränken.

Ein wesentlicher Schritt der Datenaufbereitung bestand darin, aus dem vorliegenden Zeitstempel mittels der Funktion `hour()` die volle Stunde des Unfallzeitpunkts zu extrahieren. Dies ermöglichte eine Aggregation der Daten, indem die absolute Anzahl der Unfälle pro Stunde ausgezählt wurde.

Die Visualisierung erfolgte anschließend als Balkendiagramm, das die Häufigkeit der Unfälle über den 24-Stunden-Verlauf darstellt. Um den zugrunde liegenden Trend und die Verteilungsmuster – wie den sichtbaren Anstieg in den Nachmittagsstunden – deutlicher hervorzuheben, wurde zusätzlich eine Glättungslinie (`geom_smooth` mit der Methode „loess“) über die Balken gelegt. Zur besseren Orientierung wurde die x-Achse dabei in Zwei-Stunden-Schritten skaliert.
...
Für die statistische Überprüfung der Zusammenhänge wurde der Pearson-Chi-Quadrat-Test angewendet. Im ersten Schritt wurde die Abhängigkeit zwischen der Beteiligung nicht-motorisierter Verkehrsteilnehmer und den Lichtverhältnissen untersucht. Hierfür wurden die Daten zunächst bereinigt, indem fehlende Werte in der Spalte „Light“ ausgefiltert wurden. Anschließend wurde eine Kontingenztabelle erstellt, welche die binäre Variable der Beteiligung (`has_nonmotorist`) den verschiedenen Kategorien der Lichtverhältnisse gegenüberstellt.

Ein vergleichbares Vorgehen wurde bei der Analyse der Verletzungsschwere angewendet. Der Unterschied besteht darin, dass hier die Spalte „Injury.Severity“ als Basis diente und entsprechende fehlende Werte (NA) im Vorfeld entfernt wurden. Die Durchführung des Tests erfolgte analog über die Kreuztabellierung der Variablen. In beiden Analysen weisen die extrem niedrigen p-Werte (1.591e-09 bzw. < 2.2e-16) auf einen hochsignifikanten statistischen Zusammenhang zwischen der Unfallbeteiligung von Fußgängern/Radfahrern und den untersuchten Faktoren (Lichtverhältnisse sowie Verletzungsschwere) hin.

#### Zwischenfazit Fragestellung E

Die Analyse bestätigt die aufgestellte Hypothese in wesentlichen Punkten:

**Verletzungsschwere:** Der Anteil schwerer Verletzungen (FATAL + SUSPECTED SERIOUS INJURY) bei Unfällen mit Fußgängern/Radfahrern (5,53%) ist nahezu siebenmal höher als bei reinen Fahrzeugunfällen (0,79%). Dies bestätigt eindeutig die Hypothese, dass der fehlende Schutz durch eine Karosserie diese Verkehrsteilnehmer besonders verwundbar macht.

**Lichtverhältnisse:** Entgegen der ursprünglichen Vermutung ereignen sich Unfälle mit nicht-motorisierten Beteiligten nicht häufiger bei Dunkelheit. Der Dunkelheits-Anteil liegt bei Unfällen mit Fußgängern/Radfahrern (21,3%) nur geringfügig höher als bei reinen Fahrzeugunfällen (19,7%). Die Mehrheit dieser Unfälle findet bei Tageslicht statt. Der Chi-Quadrat-Test (p = 1,59e-09) zeigt dennoch einen signifikanten Zusammenhang.

**Zeitliche Verteilung:** Die Unfälle mit nicht-motorisierten Beteiligten zeigen ein deutliches Maximum in den Nachmittagsstunden (14–18 Uhr), was mit typischen Freizeit- und Pendelzeiten für Fußgänger und Radfahrer korreliert.

**Fazit:** Die Hypothese zur erhöhten Verletzungsschwere ist vollständig bestätigt (fast 7-fach erhöhtes Risiko schwerer Verletzungen). Die Vermutung häufigerer Unfälle bei Dunkelheit trifft jedoch nicht zu – die Lichtverhältnisse unterscheiden sich nur geringfügig von reinen Fahrzeugunfällen.
...
**Ergebnis:** Die zeitliche Analyse zeigt klare, statistisch signifikante Muster. Die Rush-Hour-Peaks (7–9 Uhr und 16–18 Uhr) sind deutlich erkennbar, wobei der abendliche Peak ausgeprägter ist als der morgendliche. Werktage zeigen das klassische Pendler-Muster, während Wochenenden ein gleichmäßigeres Unfallaufkommen zur Tagesmitte aufweisen. Der Anteil schwerer Verletzungen ist nachts (20–6 Uhr) mit 1,38% fast doppelt so hoch wie zu anderen Tageszeiten (0,72–0,79%).
...
**Ergebnis:** Die Datenbasis enthält nur die generische Kategorie „VISUAL DISTRACTION" für alle Fälle – eine Differenzierung nach spezifischen Ablenkungsarten (Handy, Gespräche etc.) ist nicht möglich. Bei visueller Ablenkung dominieren Auffahrunfälle (REAR-END) mit 33% aller Kollisionen, gefolgt von Winkelkollisionen (ANGLE) mit 20%.

**Vergleich mit Hypothese:** ⚠️ **Nicht testbar / Teilweise bestätigt** — Die spezifische Hypothese zur Handynutzung kann aufgrund der Datenlimitation nicht überprüft werden. Der Zusammenhang zwischen visueller Ablenkung und Auffahrunfällen (ein Teilaspekt der ursprünglichen Vermutung) wird jedoch bestätigt.

#### Fragestellung E: Fußgänger- und Radfahrerunfälle

**Ergebnis:** Der Anteil schwerer Verletzungen bei Unfällen mit Fußgängern/Radfahrern (5,53%) ist fast siebenmal höher als bei reinen Fahrzeugunfällen (0,79%). Entgegen der Erwartung findet jedoch die Mehrheit dieser Unfälle bei Tageslicht statt; der Dunkelheits-Anteil (21,3%) liegt nur geringfügig über dem bei Fahrzeugunfällen (19,7%).

**Vergleich mit Hypothese:** ✅ **Teilweise bestätigt** — Die Hypothese zur erhöhten Verletzungsschwere ist vollständig bestätigt (7-fach erhöhtes Risiko). Die Vermutung, dass Unfälle mit Fußgängern/Radfahrern häufiger bei Dunkelheit auftreten, trifft jedoch nicht zu.
...
2. **Vulnerable Verkehrsteilnehmer sind massiv gefährdet:** Das Risiko schwerer Verletzungen für Fußgänger und Radfahrer ist fast siebenmal höher als bei Fahrzeuginsassen (5,53% vs. 0,79%). Infrastrukturmaßnahmen wie geschützte Radwege und Fußgängerüberwege könnten hier Leben retten.

3. **Zeitliche Muster ermöglichen gezielte Prävention:** Das doppelt so hohe Risiko schwerer Verletzungen in der Nacht (1,38% vs. 0,72-0,79%) sowie die Rush-Hour-Peaks bieten Ansatzpunkte für verstärkte Polizeipräsenz und Aufklärungskampagnen zu spezifischen Zeiten.

4. **Risikokompensation funktioniert – meist:** Bei Regen und Schnee ist das Risiko schwerer Verletzungen niedriger als bei klarem Wetter, was auf angepasstes Fahrverhalten hindeutet. Bei Nebel versagt dieser Mechanismus jedoch (höchste Verletzungsquote mit 1,06%).

5. **Datenlimitationen schränken manche Analysen ein:** Die fehlende Differenzierung der Ablenkungsarten verhinderte die Überprüfung der Handy-Hypothese – ein Hinweis auf Verbesserungspotenzial bei der polizeilichen Datenerfassung.
...
- **Fehlende Differenzierung:** Die Variable „Driver.Distracted.By" enthält nur die generische Kategorie „VISUAL DISTRACTION" – spezifische Ablenkungsarten wie Handynutzung konnten daher nicht analysiert werden.
